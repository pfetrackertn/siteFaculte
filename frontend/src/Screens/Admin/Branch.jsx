...
const Branch = () => {
  ...
  const addBranchHandler = async () => {
    ...
    try {
      toast.loading(isEditing ? "Mise a jour de la filiere" : "Ajout de la filiere");
      const headers = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("userToken")}`,
      };
  ...
  const confirmDelete = async () => {
    try {
      toast.loading("Suppression de la filiere");
      const headers = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("userToken")}`,
      };
      ...
      toast.dismiss();
      if (response.data.success) {
        toast.success("La filiere a ete supprimee avec succes");
  ...
  return (
    <div className="w-full mx-auto mt-10 flex justify-center items-start flex-col mb-10 relative">
      <Heading title="Gestion des filieres" />
      <CustomButton
        onClick={() => {
          ...
          <table className="text-sm min-w-full bg-white">
            <thead>
              <tr className="bg-blue-500 text-white">
                ...
                <th className="py-4 px-6 text-left font-semibold">ID de la filiere</th>
                <th className="py-4 px-6 text-left font-semibold">
                  Cree le
                </th>
      ...
      <DeleteConfirm
        ...
        message="Voulez-vous vraiment supprimer cette filiere ?"
      />
    </div>
  );
};
